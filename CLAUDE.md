...
**Container startup chain:** `init.sh` (root: resolves gateway via Docker DNS, sets routes and DNS, drops NET_ADMIN via capsh) → `entrypoint.sh` (claude user: syncs plugins, starts sshd) → SSH login triggers `tmux-picker.sh` via bashrc.
...
Shell scripts in `shared/runtime/` and `gateway/` run inside containers. Changes require rebuilding (`docker compose build`) unless using `-SandboxDev` mode which bind-mounts them. The gateway uses `/bin/sh` (Alpine/busybox ash); the claude container and picker use `/bin/bash`.
