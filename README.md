...
- Extensions (every version): `xml` (dom/simplexml/xmlwriter — hard-required by PHPUnit and most frameworks), `mbstring`, `curl`, `intl`, `bcmath`, `gmp`, `zip`, `gd`, `imagick`, `tidy`, `soap`, `apcu`, `redis`, `readline`, `pcntl`, plus database drivers for MySQL (`mysqli`/`pdo_mysql`), PostgreSQL (`pgsql`/`pdo_pgsql`) and SQLite (`sqlite3`/`pdo_sqlite`). Common built-ins (`pdo`, `phar`, `tokenizer`, `fileinfo`, `ctype`, …) come with the base packages.
...
- **Debugging & coverage:** `xdebug` and `pcov` are installed for every version. Xdebug is kept **off by default** (`xdebug.mode=off`) so it doesn't slow normal runs or change `var_dump`/error output — enable it per command with the `XDEBUG_MODE` env var (`XDEBUG_MODE=debug php …` to step-debug, `XDEBUG_MODE=develop` for richer errors). PHPUnit/Pest **line coverage works out of the box** via `pcov` (`--coverage-text`, `--coverage-html`) with no env var; use `XDEBUG_MODE=coverage` only if you need Xdebug's branch/path coverage.
